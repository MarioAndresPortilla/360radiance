...
export function WhySection() {
  return (
    <section className="py-20 max-md:py-14" aria-labelledby="why-heading">
        ...
        <div className="grid grid-cols-3 gap-6 max-lg:grid-cols-2 max-md:grid-cols-1" role="list">
          {WHY_CARDS.map((card) => (
            <ScrollReveal key={card.title}>
              <article className="group bg-white border border-border rounded-2xl p-8 transition-all duration-300 relative hover:border-border-hover hover:shadow-md hover:-translate-y-0.75" role="listitem">
                ...
                <div className={`w-13 h-13 rounded-3.5 flex items-center justify-center mb-4 ${card.iconTheme === 'teal' ? 'bg-teal-pale' : 'bg-gold-pale'}`} aria-hidden="true">
                  <Icon name={card.icon as IconName} size={26} className={card.iconTheme === 'teal' ? 'text-teal' : 'text-gold-dark'} />
                </div>
                <h3 className="font-serif text-[1.1rem] mb-2">{card.title}</h3>
                <p className="text-text-mid text-[.85rem] leading-[1.7]">{card.description}</p>
              </article>
            </ScrollReveal>
          ))}
        </div>
